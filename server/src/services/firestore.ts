...
export async function markKeyFound(sessionId: string, keyValue: string): Promise<boolean> {
  if (db) {
    ...
    try {
      ...
      if (found.includes(keyValue)) return false;
    ...
    if (found.has(keyValue)) return false;
    found.add(keyValue);
    return true;
  }
}

export async function getFoundKeys(sessionId: string): Promise<Set<string>> {
  if (db) {
...
export async function addLeaderboardEntry(entry: LeaderboardEntry): Promise<void> {
  ...
  if (db) {
    ...
    leaderboard.sort((a, b) => {
      if (a.mode === 'fastest' && b.mode === 'fastest') return a.score - b.score; // Lower time is better
