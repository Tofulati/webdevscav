...
function ensureSimulationStartMarker(html) {
    if (!/<body[^>]*>/i.test(html)) {
        return html;
    }
...
function keyPresentInGeneratedHtml(html, value) {
...
function insertAfterRandomMatch(html, re, injection) {
...
function scatterPlaintextIntoLayout(layoutHtml, chunks) {
    let out = layoutHtml;
...
function scatterOtherLeaksIntoLayout(layoutHtml, chunks) {
    let out = layoutHtml;
    for (const chunk of shuffleArray(chunks)) {
        const injection = `\n${chunk}\n`;
...
function generateKeys(count, difficultyLevel) {
    ...
    for (let i = 0; i < count; i++) {
        const sorted = [...pool].sort((a, b) => (usage.get(a.location) ?? 0) - (usage.get(b.location) ?? 0));
...
function buildLayoutStyles(variant) {
    const base = `
...
function buildLayoutBody(template, variant) {
    ...
    if (variant === 'profile') {
        ...
        const projectCards = template.feedItems
            .map((item) => `<article><img src="https://picsum.photos/seed/${encodeURIComponent(item.title + 'pf')}/560/300" alt=""><h4>${item.title}</h4><p>${item.detail}</p><span>${item.meta}</span></article>`)
            .join('');
        const metricCards = template.panelRows
            .map((row) => `<article><strong style="font-size:19px;color:var(--text);display:block;">${row.value}</strong><p style="margin-top:8px;font-size:12px;color:var(--text-muted);">${row.label}</p></article>`)
            .join('');
        const stackTags = [
            'TypeScript',
            'React',
    ...
    keys.forEach((key) => {
        ...
        switch (key.location) {
            ...
            case 'console-log':
                ...
                break;
            case 'localstorage':
                scriptFrags.push(`try{localStorage.setItem(${JSON.stringify(rndLs())},${jsFromB64(key.value)});}catch(e){}`);
                break;
            case 'script-variable':
                scriptFrags.push(`try{window[${JSON.stringify(rndWin())}]={v:${jsFromB64(key.value)}};}catch(e){}`);
                break;
            case 'cookie':
                scriptFrags.push(pick([
            ...
            case 'session-storage':
                scriptFrags.push(`try{sessionStorage.setItem(${JSON.stringify(rndSs())},${jsFromB64(key.value)});}catch(e){}`);
                break;
            case 'network-response':
                scriptFrags.push(pick([
            ...
            case 'console-invoke': {
                ...
                domLeaksOther.push(`<!-- checkout helper: window.${fn}() in console returns the promo / order reference string -->\n`);
                scriptFrags.push(`try{globalThis[${JSON.stringify(fn)}]=function(){var k=${jsFromB64(key.value)};console.info(k);return k;};}catch(e){}`);
                break;
            }
            case 'script-bundle':
                scriptFrags.push(`try{/* chunk:vendor.ops.v2 */void function(H){var u=typeof H==='string'?atob(H):'';void(u);}(${JSON.stringify(b64Utf8(key.value))});}catch(e){}`);
                break;
            case 'data-attribute':
                domLeaksOther.push(pick([
